...
def receipt(receipt_no):
    ...
    # Embed verification link with full parameters in QR — tamper-resistant verification
    verify_url = url_for('link_verify',
    ...
    qr.make(fit=True)
    img = qr.make_image(fill_color="#00d4ff", back_color="white")
    buf = io.BytesIO()
    img.save(buf, "PNG")
...
# ─── API: Student Stats ────────────────────────────────────────────────────────
@app.route('/api/receipt/<path:receipt_no>')
def api_receipt(receipt_no):
    """JSON API for receipt verification — used by QR scanner app"""
    conn = get_db()
    r = conn.execute("""
        SELECT p.*, s.fullname, s.matric, s.department, s.level, f.fee_type
        FROM payments p
    ...
    conn.close()
    if not r:
        return jsonify({'valid': False, 'message': 'Receipt not found'}), 404
    return jsonify({
        'valid': True,
        'receipt_no': r['receipt_no'],
        ...
        'amount_formatted': f"₦{r['amount_paid']:,.0f}",
        'date': r['payment_date'],
